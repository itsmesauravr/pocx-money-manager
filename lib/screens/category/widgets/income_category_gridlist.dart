...
class IncomeCategory extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    builder: (BuildContext ctx, List<CategoryModel> newList, Widget? _) {
      if (CategoryDb().incomeCategoryListListener.value.isEmpty) {
        return Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Image.asset('assest/images/add.png',width: 225,),
            const Text('No Income category added  so\nUse + for add Income category',style: TextStyle(fontWeight: FontWeight.w400,color: Colors.red),),
          ],
        );
      }
                                    ...
                                    actions: [
                                      TextButton(
                                        onPressed: (){
                                         ...
                                         TextButton(
                                        onPressed: (){
                                          CategoryDb.instance.deleteCategory(catgory.id);
                                          Navigator.of(context).pop();
                                        }, child: const Text('OK'))
                                    ],
                                  ));
